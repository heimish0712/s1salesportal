...
function getCustomerSearchIndexData() {
  const sheet = ensureCustomerSearchIndexSheet_();
  ...
  rows = filterPortalCustomerRowsByPermission_(rows);
...
function getCustomerDetailsBatch(rowNos) {
  // v45: 상세 프리패치 최적화
  // 기존 방식은 rowNo마다 getCustomerDetail(rowNo)를 반복 호출하여 매우 느렸습니다.
  // 여기서는 마스터시트를 가능한 한 번의 범위 읽기로 처리하고, 컨택이력/ raw는 생략한 lite detail을 반환합니다.
  ...
  let targets = unique.slice(0, Math.max(1, Math.min(100, limit)));
  targets = targets.filter(function(rowNo) {
    try {
      const sheetForPerm = getMasterSpreadsheet_().getSheetByName(PORTAL_CONFIG.MASTER_SHEET_NAME);
      return !!(sheetForPerm && assertPortalCanAccessCustomerTarget_({ sheet: sheetForPerm, rowNo: rowNo }, '고객 상세 프리패치'));
    } catch (err) {
      return false;
    }
  });
  ...
  // 행 간격이 너무 넓으면 불필요한 대량 읽기를 피하기 위해 개별 조회로 fallback합니다.
  if (span <= 350) {
    ...
    targets.forEach(function(rowNo) {
      try {
        ...
        const obj = buildMasterRowObjectFromValues_(headerMap, values);
        resultByRow[rowNo] = buildCustomerDetailFromObj_(obj, rowNo, { includeLogs: false, includeRaw: false, lite: true });
      } catch (err) {
  ...
  return targets.map(function(rowNo) {
    return resultByRow[rowNo] || { rowNo: rowNo, error: '상세정보를 찾지 못했습니다.' };
  });
}
